    ...
    order = function() {
      ...
      getPaymentConfig(function() {
        ...
        return ApiService.request(options, function(error, result) {
          ...
          getOrderSummary(function(error, result) {
            var ref4;
            if (error) {
              UtitService.notify(null, error.message);
            }
          ...
          return getVouchers(function(error, result) {
            var data, isWhitelist, whitelistEmails;
            if (error) {
              return;
            }
            data = result.data;
            ...
            isWhitelist = data.isWhitelist, whitelistEmails = data.whitelistEmails, result = data.result;
            if (!result || !result.length) {
              return;
            }
            $scope.promotions = result;
            if (!isWhitelist || (Array.isArray(whitelistEmails) && whitelistEmails.length && whitelistEmails.indexOf($scope.userPayment.email) !== -1)) {
              return $scope.canShowPromotions = true;
            }
          });
        });
      });
